...
#if canImport(mobile_uniffiFFI)
import mobile_uniffiFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_metasecret_mobile_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_metasecret_mobile_rustbuffer_free(self, $0) }
    }
}
...
public func uniffiMobileUpdateMembership(candidate: String, actionUpdate: String) -> String {
    ...
    uniffi_metasecret_mobile_fn_func_update_membership(
        ...
        FfiConverterString.lower(actionUpdate),$0
    )
})
}
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_metasecret_mobile_checksum_func_accept_recover() != 27303) {
    ...
    if (uniffi_metasecret_mobile_checksum_func_find_claim_id_by() != 54731) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_metasecret_mobile_checksum_func_generate_master_key() != 14660) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_metasecret_mobile_checksum_func_generate_user_creds() != 9170) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_metasecret_mobile_checksum_func_get_state() != 9387) {
        return InitializationResult.apiChecksumMismatch
    }
