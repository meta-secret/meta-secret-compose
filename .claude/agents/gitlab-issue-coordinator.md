...
> **Note:** Issues for **meta-secret-compose** are tracked on **GitHub**, not GitLab. For GitHub issues, use **`github-issue-coordinator`** at the MetaSecret workspace (via **`workflow-from-issue`** / MetaSecret **`/compose-workflow-from-issue`**) or the `workflow-from-issue` command in this repo. This agent is retained for GitLab-hosted projects only.
...
Cross-check scope against `CLAUDE.md`, `ARCHITECTURE.md`, and `SECURITY.md` (FFI boundaries, no Rust edits here).
...
4. Then **`test-verifier`** to run Gradle tests, **`code-reviewer`**, **`release-notes`**, and **`release-manager`** as needed—each step after user approval where required.
...
- Do **not** modify Rust in this repository.
