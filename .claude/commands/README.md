# meta-secret-compose — slash commands (help catalog)

Use this file when the **workspace root is this repository** (`meta-secret-compose/`). Slash names here are **`/only-*`**, **`/only-issue-coordinator`**, **`/only-from-prompt`** (no `compose-` prefix).

**Canonical** behavior and arguments live in each `*.md` file in this folder. See [WORKFLOW.md](../../WORKFLOW.md) for the full pipeline.

**MetaSecret parent workspace:** If your editor root is the **MetaSecret** folder that contains `meta-secret-core/` and `meta-secret-compose/`, use the prefixed wrappers instead (`/compose-only-issue-coordinator`, `/compose-only-planner`, …). Those are documented in the MetaSecret workspace `.claude/commands/README.md` (not in this repo).
...
|---------------|-------------|
| `/only-issue-coordinator` | `gh` fetch + structured issue **Summary** (Markdown, emoji sections); stop for user approval before planning; typical next step **`/only-planner`** (KMM app repo). |
...
| `/only-test-verifier` | Agent mode: run Gradle/KMP test tasks (scope per PROJECT_CONTEXT) and report pass/fail. |
