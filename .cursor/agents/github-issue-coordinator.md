...
2. Resolve **`owner/repo`** from `git remote` or default **`meta-secret/meta-secret-compose`**.
...
6. Next: user approves Summary → **`/only-planner`** (or MetaSecret **`/compose-only-planner`**).
...
- Do **not** modify Rust or native binaries in this repository.
