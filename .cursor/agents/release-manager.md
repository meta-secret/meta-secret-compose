...
Operate **only** inside this repository (`meta-secret-compose`). Base branch for new work: **`main`**.
...
1. Confirm current `git remote` and that the working tree is the compose repo root.
...
- Do **not** change signing, provisioning, or team settings.
- Do **not** touch Rust sources in this repo.
