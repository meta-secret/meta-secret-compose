...
|--------------------|-------------------|--------------|
| `/compose-only-release-manager` | `/only-release-manager` | Branch, commit, push, MR — **only after explicit user ok** |
| `/compose-only-reviewer` | `/only-reviewer` | Final review before merge |

Typical next step after notes are approved: **`/compose-only-release-manager`** (MetaSecret) or **`/only-release-manager`** (repo root).
