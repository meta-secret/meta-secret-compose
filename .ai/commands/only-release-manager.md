...
|--------------------|-------------------|--------------|
| `/compose-only-workflow-pattern-capture` | `/only-workflow-pattern-capture` | Optional: capture repeated process learnings |
| `/compose-only-release-notes` | `/only-release-notes` | Refine MR body if needed |

Typical next step after MR is open: optional **`/compose-only-workflow-pattern-capture`**, or close the task per team process.
